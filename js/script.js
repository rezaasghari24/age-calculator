...
const resolveDateIssues = function () {
    for (let i = 0 ; i < birthDate.length - 1; i++) {
        if(birthDate[i] > currDate[i] && i === 0) { // Check day
            currDate[i + 1] -= 1;
            currDate[i] += 30;
        } else if (birthDate[i] > currDate[i] && i === 1) { // Check month
                currDate[i + 1] -= 1;
                currDate[i] += 12;
            }
    }
